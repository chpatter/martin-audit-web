...
// ─── Vendor Pricing module config ───

const SOURCE_TABLES = [
  { key: 'PDSV', color: '#60a5fa' },
];

const SOURCE_OPTIONS = [
  { value: 'pdsv', label: 'PDSV (Vendor Pricing)' },
];

const FILTER_KEYS = ['source', 'startdt', 'enddt', 'prod', 'vendname', 'field', 'transproc', 'new_value', 'old_value', 'transdt', 'effectiveEnd', 'oper', 'description'];
...
const FILTER_LABELS = {
  ...
  field: 'Vendor Price Changes',
  transproc: 'Transaction Process',
  new_value: 'Vendor Price New Value',
  old_value: 'Vendor Price Original Value',
...
const COLUMNS = [
  ...
  { key: 'field_label', label: 'Vendor Price Changes', width: 240 },
  { key: 'transproc', label: 'Transaction Process', width: 150 },
  { key: 'new_value', label: 'Vendor Price New Value', width: 190 },
  { key: 'old_value', label: 'Vendor Price Original Value', width: 190 },
...
const CSV_HEADERS = [
  'Data Source', 'Start Date', 'End Date', 'Product',
  'Vendor Name & Number', 'Vendor Price Changes', 'Transaction Process',
  'Vendor Price New Value', 'Vendor Price Original Value',
...
function csvRowMapper(row) {
  return [
    row.source, row.startdt || '', row.enddt || '', row.prod || '',
    row.vendname || '', `${row.field_label} - (${row.field_name})`, row.transproc || '',
    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {changes.length > 0 && (
            <input placeholder="Filter results... (e.g. prcmult, qtybrk, refer)" value={resultFilter}
