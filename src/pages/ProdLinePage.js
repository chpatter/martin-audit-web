...
// ─── Product Line module config ───

const SOURCE_TABLES = [
  { key: 'ICSL', color: '#34d399' },
];

const SOURCE_OPTIONS = [
  { value: 'icsl', label: 'ICSL (Product Line Setup)' },
];
...
const COLUMNS = [
  ...
  { key: 'pono', label: 'Product Line', width: 120 },
  { key: 'lineno', label: 'Warehouse', width: 80 },
  { key: 'vendname', label: 'Vendor Name & Number', width: 210 },
  { key: 'field_label', label: 'Product Line Changes', width: 220 },
  { key: 'transproc', label: 'Transaction Process', width: 150 },
  { key: 'new_value', label: 'New Value', width: 180 },
  { key: 'old_value', label: 'Original Value', width: 180 },
...
const CSV_HEADERS = [
  'Data Source', 'Product Line', 'Warehouse', 'Vendor Name & Number',
  'Product Line Changes', 'Transaction Process', 'New Value',
...
function csvRowMapper(row) {
  return [
    ...
    row.vendname || '', `${row.field_label} - (${row.field_name})`,
    row.transproc || '', row.new_value || '', row.old_value || '',
  ...
  return (
    <>
      ...
      <FilterBar
        ...
        recordLabel="Product Line" recordPlaceholder="ACO998, BRA200" sourceOptions={SOURCE_OPTIONS}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {changes.length > 0 && (
            <input placeholder="Filter results... (e.g. buyer, orderpt, vendno)" value={resultFilter}
