...
const SOURCE_TABLES = [
  { key: 'ICSC', color: '#60a5fa' },
];

const SOURCE_OPTIONS = [
  { value: 'icsc', label: 'ICSC (Catalog)' },
];

const FILTER_KEYS = ['source', 'record', 'vendname', 'field', 'transproc', 'new_value', 'old_value', 'transdt', 'effectiveEnd', 'oper', 'description'];
...
const FILTER_LABELS = {
  ...
  new_value: 'Catalog New Value',
  old_value: 'Catalog Original Value',
...
const COLUMNS = [
  ...
  { key: 'pono', label: 'Catalog', width: 100 },
  { key: 'vendname', label: 'Vendor Name & Number', width: 210 },
  { key: 'field_label', label: 'Catalog Changes', width: 240 },
  { key: 'transproc', label: 'Transaction Process', width: 150 },
  { key: 'new_value', label: 'Catalog New Value', width: 190 },
  { key: 'old_value', label: 'Catalog Original Value', width: 190 },
...
const CSV_HEADERS = [
  'Data Source', 'Catalog', 'Vendor Name & Number',
  'Catalog Changes', 'Transaction Process', 'Catalog New Value',
  'Catalog Original Value', 'Effective Start DateTime', 'Effective End DateTime',
  'User and OperID', 'description',
];

function csvRowMapper(row) {
  return [
    row.source, row.pono, row.vendname || '',
    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {changes.length > 0 && (
            <input placeholder="Filter results... (e.g. vendprod, listprice, prodline)" value={resultFilter}
