...
// ─── Transfers module config ───

const SOURCE_TABLES = [
  { key: 'WTEH', color: '#60a5fa' },
  { key: 'WTEL', color: '#fbbf24' },
];

const SOURCE_OPTIONS = [
  { value: 'wteh', label: 'WTEH (Transfer Header)' },
  { value: 'wtel', label: 'WTEL (Transfer Lines)' },
];
...
const FILTER_LABELS = {
  ...
  lineno: 'Line No',
  whse: 'shiptowhse',
  field: 'Transfer Changes',
  transproc: 'Transaction Process',
  new_value: 'Transfer New Value',
  old_value: 'Transfer Original Value',
...
const COLUMNS = [
  ...
  { key: 'pono', label: 'Full Transfer', width: 100 },
  { key: 'lineno', label: 'Line No', width: 60 },
  { key: 'whse', label: 'shiptowhse', width: 80 },
  { key: 'field_label', label: 'Transfer Changes', width: 240 },
  { key: 'transproc', label: 'Transaction Process', width: 150 },
  { key: 'new_value', label: 'Transfer New Value', width: 190 },
  { key: 'old_value', label: 'Transfer Original Value', width: 190 },
...
const CSV_HEADERS = [
  'Data Source', 'Full Transfer', 'Line No', 'shiptowhse',
  'Transfer Changes', 'Transaction Process', 'Transfer New Value',
  'Transfer Original Value', 'Effective Start DateTime', 'Effective End DateTime',
  'User and OperID', 'description',
];

function csvRowMapper(row) {
  return [
    row.source, `${row.pono}-${row.posuf}`, row.lineno || 0, row.whse || '',
  ...
  return (
    <>
      ...
      <FilterBar
        ...
        recordTooltip="Search by warehouse transfer number. Add a suffix with a dash (e.g. 32836-1) to search a specific suffix."
        showOperator
      />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {changes.length > 0 && (
            <input placeholder="Filter results... (e.g. stagecd, qtyord, shipprod)" value={resultFilter}
