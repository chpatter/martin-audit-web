 ...
 * Fetches the operator list once from /api/operators and caches it.
 * User can type to filter by code or name, or pick from the dropdown.
 */

let cachedOperators = null;

export default function OperatorSelect({ value, onChange, onSearch }) {
  const { theme } = useTheme();
  const [open, setOpen] = useState(false);
  const [operators, setOperators] = useState(cachedOperators || []);
